...
def get_CUI_source_types(query_name: str, umls_token: str):

    url = f"https://uts-ws.nlm.nih.gov/rest/content/current/CUI/{query_name}/relations?apiKey={umls_token}&pageSize=100"
...
def get_name(umls: str, umls_token: str):
    url = f"https://uts-ws.nlm.nih.gov/rest/content/current/CUI/{umls}?apiKey={umls_token}"
...
def get_cui_from_subcategory(code: str, subcategory: str, umls_token: str):
    url = f"https://uts-ws.nlm.nih.gov/rest/content/current/source/{subcategory}/{code}/atoms/preferred?apiKey={umls_token}"
    response = requests.get(url).json()
    ...
    if "status" in response and str(response["status"]) == "404":
        ...
        if "result" in response:
            ...
            r = response["result"]
            if "concept" in r and "/CUI/" in r["concept"] and "name" in r:
        
                result["cui"] = r["concept"].split("/")[-1]
                result["name"] = r["name"]
...
def get_MOA_cui(medrt: str, umls_token: str):
    url = f"https://uts-ws.nlm.nih.gov/rest/content/current/source/MED-RT/{medrt}/atoms/preferred?apiKey={umls_token}"
    response = requests.get(url).json()
...
###
    ...
    #print (get_subcategory_id("C0032825", "RXNORM", umls_token))
    #print (get_name("C0935942", umls_token))
    print (get_MOA_cui("N0000000152", umls_token))